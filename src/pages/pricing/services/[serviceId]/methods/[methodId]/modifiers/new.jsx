...
const NewModifier = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await modifiersController.createModifier(payload)

      if (res.success) {
        toast.success('Modifier created successfully')
        router.push(`/pricing/services/${serviceId}/methods/${methodId}/modifiers`)
      } else {
        notify(res.error?.message || 'Failed to create modifier')
      }
    } catch (error) {
      console.error('Error creating modifier:', error)
      notify('Failed to create modifier')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className={styles.modifierForm}>
      <PageHead
        ...
        current="Create New Modifier"
      >
        <Head
          title={`Create New Modifier${pricingMethod ? ` - ${pricingMethod.name}` : ''}`}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                ...
                <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.875rem', color: '#666' }}>
                  Creating a new modifier for this pricing method
                </p>
              </div>
            </div>
          </Card>
          ...
          <form onSubmit={handleSubmit} className={styles.form}>
            ...
            <div className={styles.actions}>
              ...
              <Button type="submit" primary disabled={submitting}>
                {submitting ? 'Creating...' : 'Create Modifier'}
              </Button>
            </div>
          </form>
        </Card>
